  ...
  def chunk_report_data_by_size(%ReportData{} = report, max_bytes) when max_bytes > 0 do
    ...
    case pack_by_size(report.attribute_reports, template, overhead, max_bytes) do
      ...
      groups ->
        ...
        init_reports ++ [%ReportData{report | attribute_reports: last, more_chunked_messages: false}]
    end
  end
    ...
    {groups, current, _running} =
      Enum.reduce(reports, {[], [], overhead}, fn r, {groups, current, running} ->
        ...
        cond do
          ...
          current == [] -> {groups, [r], overhead + size_r}
          running + size_r > max_bytes -> {[Enum.reverse(current) | groups], [r], overhead + size_r}
          true -> {groups, [r | current], running + size_r}
        end
      end)

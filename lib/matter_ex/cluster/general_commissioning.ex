defmodule MatterEx.Cluster.GeneralCommissioning do
  ...
  attribute(0x0001, :basic_commissioning_info, :struct,
    default: %{0 => {:uint16, 120}, 1 => {:uint16, 900}}
  )
  ...
  attribute(0x000C, :is_commissioning_without_power, :boolean, default: false)
  ...
  command(
    ...
    [new_regulatory_config: :uint8, country_code: :string, breadcrumb: :uint64],
    response_id: 0x03
  )
  ...
  def handle_command(:arm_fail_safe, params, state) do
    ...
    {:ok, commissioning_response(0, ""), state}
  end
  ...
  defp commissioning_response(error_code, debug_text) do
    %{0 => {:uint8, error_code}, 1 => {:string, debug_text}}
  end
end

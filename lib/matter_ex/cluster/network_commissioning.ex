defmodule MatterEx.Cluster.NetworkCommissioning do
  @moduledoc """
  Matter Network Commissioning cluster (0x0031).

  Stub implementation for a Wi-Fi device. Commands return hardcoded success
  responses so commissioning flows can exercise Network Commissioning.
  All commands return hardcoded success responses. Endpoint 0 only.
  """

  use MatterEx.Cluster, id: 0x0031, name: :network_commissioning
  ...
  def handle_command(:scan_networks, _params, state) do
    # NetworkScanResponse: NetworkingStatus=Success, DebugText="", WiFiScanResults=[]
    {:ok, %{0 => {:uint, 0}, 1 => {:string, ""}, 2 => {:array, []}}, state}
  end

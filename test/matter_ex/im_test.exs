defmodule MatterEx.IMTest do
  ...
  defp data_reports(n) do
    for i <- 1..n do
      {:data, %{version: 1, path: %{endpoint: 1, cluster: 0x0006, attribute: i}, value: {:uint, i}}}
    end
  end
  ...
  describe "chunk_report_data_by_size/2" do
    ...
    test "a single report larger than the budget is emitted as its own chunk" do
      ...
      small =
        {:data, %{version: 1, path: %{endpoint: 1, cluster: 0x0006, attribute: 1}, value: {:uint, 1}}}
  ...
  describe "error handling" do
    test "unknown opcode" do
      binary = MatterEx.TLV.encode(%{0 => {:uint, 0}})
